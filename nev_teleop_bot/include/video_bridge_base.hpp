...
public:
  VideoBridgeBase(
    const std::string & node_name, const std::string & codec,
...
private:
  void init_zenoh()
  {
    ...
    RCLCPP_INFO(
      get_logger(), "Zenoh → %s",
      zenoh_locator_.empty() ? "auto" : zenoh_locator_.c_str());
  ...
  void on_image(const ImageMsg::SharedPtr msg)
  {
    ...
    if (now - log_time_ >= 2.0) {
      RCLCPP_INFO(
        get_logger(), "[enc] cb=%.1ffps out=%d drop=%d last_dt=%.0fms",
  ...
  {
    auto b = [this](const char * p) -> const char * {
        return get_parameter(p).as_bool() ? "true" : "false";
      };
    ...
    RCLCPP_INFO(
      get_logger(), "[pipeline] codec=%s hw_accel=%s encoder=%s",
    ...
    if (hw_accel) {
      ...
      if (factory) {
        ...
        RCLCPP_ERROR(
          get_logger(), "[pipeline] encoder '%s' NOT found — hw_accel will fail",
