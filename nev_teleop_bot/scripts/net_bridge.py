...
class NetBridge(Node):
    def __init__(self):
        ...
        self._zsubs = [
            ...
            self._zsession.declare_subscriber('nev/gcs/ping',     self._on_ping),
        ]
    ...
    def _on_ping(self, sample):
        try:
            ...
            if ts is None:
                return
            self._zpubs['nev/robot/pong'].put(json.dumps({'ts': ts}))
        except Exception as e:
            self.get_logger().warning(f'ping parse error: {e}')
